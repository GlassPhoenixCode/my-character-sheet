...
import { dmPartyEntriesToCreatures, readDmPartyFromLocalStorage } from "../../data/dmPartyImport"
...
const Simulation:FC<PropType> = ({}) => {
    ...
    const [dmPartyPlayers, setDmPartyPlayers] = useState<Creature[]>([])
    ...
    useEffect(() => {
        setDmPartyPlayers(readDmPartyFromLocalStorage())

        function onMessage(event: MessageEvent) {
            if (event.data?.type !== 'dm-party') return
            setDmPartyPlayers(dmPartyEntriesToCreatures(event.data.party))
        }
        ...
        <div className={styles.simulation}>
            <semiPersistentContext.Provider value={{state, setState}}>
                <EncounterForm
                    ...
                    setLuck={setLuck}>
                        <>
                            { dmPartyPlayers.length ? (
                                <button onClick={() => setPlayers(dmPartyPlayers)}>
